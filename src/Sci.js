...
export default class Sci extends Component {
    article=[
        {
        "source": {
       "id": "cbs news",
    ...
    async componentDidMount(props){
      ...
      let url=`https://newsapi.org/v2/top-headlines?country=in&category=science&apiKey=3dfdb9102b154deab66608dfeb6457b9&page=science&pageSize=5`
      let data= await fetch(url)
      let pars= await data.json()
      this.setState({article:pars.articles,total:pars.totalResults})
    ...
    fetchMoreData=async ()=>{
      ...
      let url=`https://newsapi.org/v2/top-headlines?country=in&category=science&apiKey=3dfdb9102b154deab66608dfeb6457b9&page=${this.state.page}&pageSize=5`
      this.setState({loading:true})
      this.setState({page:this.state.page+1})
      let data= await fetch(url)
      let pars= await data.json()
      this.setState({article:this.state.article.concat(pars.articles),total:pars.totalResults})
